...
%         set(Ts.Children, 'fontsize', 11)
    ...
    function dataR = allP_hist2d_rho_tw_v_iw_effectsize(F, metric, thresh, min_electrodes)
    ...
    %     Q = quantile(proportions, ptest, 3);
        Q = min(proportions, [], 3);
        mn = mean(proportions, 3); sd = std(proportions, [], 3);
        z = mn ./ sd;
...
%             drawnow;
    ...
    function compare_metrics(F, pat, metrics)
        ...
        % Labels
        title(T, F.SeizureInfo.display_names{sz_num})
...
%                 if isempty(iw_info.center), continue; end
...
%             data_all.ubo);    %#ok<ASGLU>
    ...
    function make(F, patients, style)
        ...
        % Here we show that 
        F.allP_hist2d_rho_tw_v_iw_effectsize('M', 5e-2, MIN_ELEC);
        F.allP_quantiles_pval_tw_v_iw;
        F.allP_large_shift_counts;
        
        

        
        
        % The min electrodes doesn't matter that much here. It's set to 30
        % since this way the placement should match the first run but
        % you'll just have to remask if you change it.
        F.allP_iw_stats(30); 
        ...
        F.allP_quantiles_pval_tw_v_iw([], [], 40);
        
        % Show correlation v. time (relative to IW)
        F.allP_corr_v_time('M', 5e-2, 40);
        
        % Test for times of significance (this one takes a while)
        F.allP_hist2d_rho_tw_v_iw('M', 5e-2, 40)
