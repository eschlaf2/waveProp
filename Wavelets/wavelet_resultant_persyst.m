%
% Computes length of the resultant of the wavelet time-frequency decomposition from a
% persyst file, locked on event codes form a .evt file
% 
% Julien Besle 02/XX/2008

function wavelet_resultant = wavelet_resultant_persyst(event_file, event_codes, low_frequency, high_frequency, start_latency, stop_latency)

wavelet_resultant = [];

if nargin ~= 4 && nargin ~= 6
    fprintf (1, 'Usage: wavelet_resultant_persyst ( event_file, [vector of event_codess], low_frequency, high_frequency, start_latency (in ms), stop latency(in ms))\n');
    return;
end
...
wavelet_resultant.samp_rate = samp_rate;
wavelet_resultant.zero = (-1) * round (start_latency * samp_rate / 1000) + 1;
wavelet_resultant.event_codes = event_codes;
wavelet_resultant.event_names = event_names(event_codes);
wavelet_resultant.Xunit = 'ms';
wavelet_resultant.Yunit = 'Hz';
%wavelet_resultant.baseline_start = (start_baseline-start_latency)*wavelet_resultant.samp_rate/1000+1;
%wavelet_resultant.baseline_stop = (stop_baseline-start_latency)*wavelet_resultant.samp_rate/1000+1;
wavelet_resultant.name = [event_file ' : resultant of wavelet decomposition'];
wavelet_resultant.chanlist = fileinfo(1).chanlist;
        ...
        for i_trials = 1:length(trials_times)
            ...
            if size(eeg.eeg_data,2) == number_of_samples %we want only complete trials
                for i_channel = 1:size(eeg.eeg_data,1)
                    ...
                    wavelet(i_channel,:,:) = wave; 
                end
                if total_trials(i_event) == 0
                    sum = zeros(size(wavelet));
                end

                sum = sum + wavelet;%compute the sum
                total_trials(i_event) = total_trials(i_event) + 1;% the nmber of trials in the sum
            end
            clear eeg.eeg_data
%            plot(wavelet_resultant.data(71,:,i_event),'k');
        end
%        fprintf(1,'\n\n');
    end
    
    %divide by the number of trials 
    wavelet_resultant.data(:,:,:,i_event) = abs(sum) / total_trials(i_event);
    %hold off
    close(h_waitbar);
end
wavelet_resultant.number_of_trials = total_trials;
wavelet_resultant.cone_of_influence = 1./cone_of_influence;
